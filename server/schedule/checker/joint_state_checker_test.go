...
func TestBeforeSpiltLeaveJointState(t *testing.T) {
	re := require.New(t)
	...
	ruleManager.SetRule(&placement.Rule{
		GroupID:     "tiflash",
		ID:          "tiflash-1",
		StartKeyHex: "7480000000000025ff605f720000000000fa",
		EndKeyHex:   "7480000000000025ff6100000000000000f8",
	...
	sk, _ := hex.DecodeString("7480000000000025FF6000000000000000F8")
	ek, _ := hex.DecodeString("7480000000000025FF6500000000000000F8")
	for _, testCase := range testCases {
		region := core.NewRegionInfo(&metapb.Region{Id: 1, Peers: testCase.Peers}, testCase.Peers[0],
			core.WithStartKey(sk), core.WithEndKey(ek))
		op := jsc.Check(region)
		checkSteps(re, op, testCase.OpSteps)
	}
}
